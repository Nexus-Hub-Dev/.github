...
function renderSvg(entries, totalBytes) {
  ...
  const pctWidth = 66;    // espaço reservado pra porcentagem, fora da barra
  const trackGap = 6;     // folga extra entre o fim da trilha e o texto
  ...
  entries.forEach(([lang, bytes], i) => {
    ...
    // nunca deixa a barra colorida encostar no fim da trilha
    const barWidth = Math.min(
      trackWidth - trackGap,
      Math.max(2, (bytes / entries[0][1]) * trackWidth)
    );
    ...
    <text x="${trackX + trackWidth + trackGap}" y="${y + barHeight}" font-family="Segoe UI, Ubuntu, sans-serif" font-size="12" fill="#c9d1d9">${pct}%</text>`;
  });
